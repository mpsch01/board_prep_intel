...
## Active Memory Files
- [project_session_log.md](project_session_log.md) — Module state, PDF counts, key numbers, deferred flags, Intelligence 2.0 layer status — updated BATON 075 (corpus-integrity-qc V1 testing pass + DB-write debut: 1,914 Tier 1 SQL applied via inline fix-applier with 172 MB backup + atomic BEGIN/COMMIT + 6 verify COUNTs; findings 2,538 → 624; Layer C 1,798 → 0; +1 question recovered (QID-2024-0067 acute HIV) from source PDFs; 3 substantive skill bugs fixed in-flight (path off-by-one, Windows cp1252, JSON-choices SQL escape); 3 deferred flags CLOSED, 4 NEW opened)
- [project_current_db_state.md](project_current_db_state.md) — DB table row counts updated BATON 075: 2,206 articles (row count unchanged but cache columns corrected), 1,**640** ITE Qs (+1 QID-2024-0067), 1,221 AAFP, 2,710 qid_art_xref (row count unchanged, no INSERTs/DELETEs in Tier 1); DB invariants now hold (sum_citation_count == COUNT(xref) == 2,710; articles_with_citations == distinct article_ids in xref == 1,982; orphan_xref_rows == 0); pre-apply backup at `ite_intelligence.db.pre_qc_2026-05-18-221606.bak` (172 MB)
- [project_corpus_integrity_qc.md](project_corpus_integrity_qc.md) — Corpus integrity QC skill. **V1.0.1 hardened BATON 075** — first standalone production run exercised end-to-end; 3 bugs caught + fixed (PROJECT_ROOT off-by-one in all 5 entry-point scripts → 4 hops not 5; Windows cp1252 console crash on `✓` → new `setup_utf8_stdout()` in utils.py + UTF-8 to 8 open() + 3 subprocess.run() calls; A1 ENCODING_ARTIFACT no-op on JSON `choices` column — SQLite DOES interpret `\u` escapes in single-quoted string literals despite docs, fixed via `_sql_json_escape_expr()` building `char(92) || 'uXXXX'`). fix-applier first-use proven (backup + atomic transaction + COUNT verification). Layer C cleared 1,797 → 0 cache-drift findings. A4 PDF-diff + new A5 (Claude-API spell check) + A6 (DOCX render-fidelity) deferred to V1.1/V1.2.
