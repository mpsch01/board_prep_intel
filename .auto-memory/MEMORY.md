...
## Active Memory Files
- [project_session_log.md](project_session_log.md) — Module state, PDF counts, key numbers, deferred flags, Intelligence 2.0 layer status — updated BATON 077 (resident re-run + blueprint parser fix: Q17 stem recovery; encoding-table extensions + DB-fix; **Fix #1** parse_blueprint multi-page — legacy 2-page reports were dropping ~37% of items, 2022 123→195 / 2023 125→198; **Option C** Stage 1.8 DB-derived blueprint + cross-check; MAJOR FINDING old-format 2022/2023 blueprint ~70% mis-categorized; QID-2024-0017 corrected per ABFM tie-breaker; all 7 reports re-run, 0 disagreements; closed DEFERRED-RESIDENT-ANALYSES-RERUN, opened 5; no new scripts)
- [project_current_db_state.md](project_current_db_state.md) — DB table row counts unchanged BATON 077 (2,206 articles, 1,640 ITE Qs, 1,221 AAFP, 2,710 qid_art_xref, etc.); 3 write workflows (37 question rows: 1 stem recovery + 34 encoding + 1 blueprint), all invariants preserved; residual dot-leader/`Ã—` 0; blueprint disagreements 0 across all 7 reports; 9 backup files in `00_database/db/` (gitignored, prune 075/076 next session); Next ART-ID ART-2219
