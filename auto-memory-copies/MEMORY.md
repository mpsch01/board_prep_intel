...
## Active Memory Files
- [project_session_log.md](project_session_log.md) — Module state, PDF counts, key numbers, deferred flags, Intelligence 2.0 layer status — updated BATON 076 (Tier 2 walk + corpus-wide cleanup: 8 atomic DB-write workflows applied; A3 choices_empty re-extraction for 42 QIDs; QID-2024-0067 enrichment backfill; question_text embedded-choice-block cleanup for 42 questions; corpus-wide wandering-subscript orphan cleanup for 206 questions with 14 medical-knowledge recovery rules; final fidelity metrics all at 0 — was 47+117+42+42+41 across 5 categories; 5 new M2 scripts; 4 deferred flags closed + 2 opened)
- [project_current_db_state.md](project_current_db_state.md) — DB table row counts unchanged BATON 076 (all 13 tracked tables stable: 2,206 articles, 1,640 ITE Qs, 1,221 AAFP, 2,710 qid_art_xref, etc.); column values updated across ~250 questions via 8 atomic workflows; all BATON 075 invariants preserved on each apply; 5 backup files in `00_database/db/` (gitignored); Next ART-ID corrected to ART-2219 (recon found 10 articles already at ART-2208–ART-2218)
- [project_corpus_integrity_qc.md](project_corpus_integrity_qc.md) — Corpus integrity QC skill. **V1.0.1 hardened BATON 075** — first standalone production run; 3 bugs caught + fixed. **V1.1 designs proven BATON 076** — the targeted re-extractors (reextract_a3_choices.py, reextract_a2_explanations.py) prototype Layer A4 PDF-diff infrastructure; render_partial_4of5_docx.py prototypes Layer A6 RENDER_FIDELITY. **2 new layer checks queued for V1.1** (DEFERRED-CORPUS-QC-LAYER-A7-A8): A7-EMBEDDED_CHOICES_IN_STEM (would have caught BATON 076's 42 embedded-choice-block findings), A8-WANDERING_SUBSCRIPT (would have caught BATON 076's 162 subscript orphans). Both auto-detectable patterns fit existing Tier 1 auto-safe SQL pattern. A4 PDF-diff + A5 (Claude-API spell check) + A6 (DOCX render-fidelity) deferred to V1.1/V1.2.
