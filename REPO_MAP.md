...
**Last Updated:** 2026-05-19 (BATON 077 — resident re-run + blueprint parser fix + encoding DB cleanup. Reviewed a Mac QC-handoff, recovered Q17 (QID-2022-0097 stem from 2022_MC.pdf), extended encoding tables (fixed dead `_DOTLEADER` regex + `Ã?` family in build_custom_question_set.py + corpus-qc utils.py; commit f091abd), then re-ran all 7 resident analyses — which surfaced + fixed two latent bugs (commit ae47b77): **Fix #1** parse_blueprint only read page 1 of legacy 2-page (2018-2023) reports, dropping ~37% of items (2022 123→195, 2023 125→198) — now loops all pages; **Option C** Stage 1.8 blueprint now DB-derived by QID (authoritative) with PDF cross-check flagging true disagreements. MAJOR FINDING: old-format 2022/2023 blueprint ~70% mis-categorized by PDF-column geometry (differs across ABFM eras); new-format years agree 0-1/195, validating DB+QID mapping. Encoding DB-fix: dot-leader `ï€®`→`": "` + `Ã?` family across 34 rows (gitignored DB; residual 0). QID-2024-0017 blueprint corrected `Acute Care and Diagnosis`→`Chronic Care Management` per ABFM tie-breaker → all 7 reports 0 disagreements. UTF-8 console crash fixed. All 7 reports regenerated correct. Closed DEFERRED-RESIDENT-ANALYSES-RERUN; opened 5 (BLUEPRINT-XCHECK-AUTOFIX, PATIENT-BASED-SYSTEMS-RESIDUAL, DOUBLED-QUOTE, C1-CONTROLS, REPORT-BUILDER-ENCODING). No new scripts (all edits to existing files). No PDF changes. Active BATON: `BATON_active_077_20260519_resident_rerun_blueprint_fix.md`.)
...
├── BATON_active_077_*.md              Active session handoff — read first every session
