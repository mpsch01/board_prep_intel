...
**Last Updated:** 2026-05-19 (BATON 075 — Corpus-integrity-qc V1 testing pass + DB-write debut. First standalone production run of `run_qc.py` end-to-end; 3 substantive bugs in the skill caught and fixed in-flight (PROJECT_ROOT off-by-one in all 5 entry-point scripts; Windows cp1252 console crash on `✓` — new `setup_utf8_stdout()` in utils.py + UTF-8 to 8 open() + 3 subprocess.run() calls; A1 ENCODING_ARTIFACT no-op on JSON choices column — SQLite interprets `\u` escapes despite docs, fixed via `_sql_json_escape_expr()` building `char(92) || 'uXXXX'`). 1,914 Tier 1 statements applied via inline fix-applier (atomic BEGIN/COMMIT, 172 MB backup, 6 verify COUNTs). Findings 2,538 → 624; Layer C 1,798 → 0. QID-2024-0067 (acute HIV diagnostic) recovered from source PDFs after being found dropped during ingestion; inserted with primary fields populated (questions 1,639 → 1,640). DB invariants now hold. 3 deferred flags CLOSED (DEFERRED-LAYER-C-CACHE-REBUILD, DEFERRED-ORPHAN-XREF-QID-2024-0067, DEFERRED-V1-ENCODING-CHOICES-JSON-BUG opened+closed same session); 4 NEW (A5 language-integrity, A6 render-fidelity, QID-2024-0067 enrichment backfill, B-umbrella-promotion-review). 8 corpus-integrity-qc files MODIFIED (7 scripts/*.py + SKILL.md). No M1/M2/M3/M4/M5 script changes. No PDF changes. Active BATON: `BATON_active_075_20260519_corpus_qc_v1_testing_pass.md`.)
...
├── BATON_active_075_*.md              Active session handoff — read first every session
