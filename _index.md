...
**Last Updated:** 2026-05-19 (BATON 077 — resident re-run + blueprint parser fix + encoding DB cleanup. Reviewed a Mac QC-handoff, recovered Q17 (QID-2022-0097 stem from 2022_MC.pdf), extended encoding tables (dead `_DOTLEADER` regex fixed + `Ã?` family in `build_custom_question_set.py` + corpus-qc `utils.py`), then re-ran all 7 resident analyses — which surfaced + fixed two latent bugs: (a) **Fix #1** `parse_blueprint` only read page 1 of legacy 2-page reports → 2022 123→195, 2023 125→198 items; (b) **Option C** Stage 1.8 blueprint now DB-derived by QID (authoritative) with PDF cross-check flagging true disagreements. MAJOR FINDING: old-format 2022/2023 blueprint was ~70% mis-categorized by PDF-column geometry; new-format 0–1/195 validates DB+mapping. Encoding DB-fix cleaned dot-leader `ï€®`→`": "` + `Ã?` family across 34 rows (gitignored DB). QID-2024-0017 blueprint corrected Acute→Chronic Care Management per ABFM tie-breaker → all 7 reports 0 disagreements. UTF-8 console crash fixed. Closed DEFERRED-RESIDENT-ANALYSES-RERUN. Commits f091abd + ae47b77. Row counts unchanged (2206/1640/2710).)
**Status:** Current — Mac local PDFs lag Windows canonical by 567 files (tier counts unchanged BATON 077: VC_fail 1056 / VC_pass 309 / local_lite 117 / right_click 58 / AAFP 15 / exams 16); M1 build = 8 + maintain = 38; M2 = **80py** + 6js + 1json (scripts/) + 4py (core/) + 7py (engines/) + 6py (utils/) + 1py (M2 root) — no new scripts BATON 077 (edited build_custom_question_set.py + corpus-qc utils.py); M3 = 55py + 4js + 7 json config — no new scripts (edited ite_parser.py + ite_analyze_v2.py); M4 = 1py; M5 scaffold = 3py + 35ts/tsx + 5sql; DB UPDATED (37 question rows touched: 1 stem recovery + 34 encoding + 1 blueprint, row counts on all tracked tables unchanged from BATON 076 — only column values updated); `.claude/skills/` exposes **10 project-level entries**; `~/.claude/skills/` empty.
...
```
board_prep_intel/
├── BATON_active_072_20260518_device_handoff_pause.md  — active BATON
...
**Fixed (2026-05-19, BATON 077):** Resident re-run + blueprint parser fix + encoding DB cleanup. (1) Q17 recovery — QID-2022-0097 truncated stem restored from 2022_MC.pdf p.34. (2) Encoding-table extensions — fixed the dead `_DOTLEADER` regex (was targeting raw PUA codepoints; DB stores triple-mojibake `ï€®`) + added the `Ã?` double-encoding family to `build_custom_question_set.py` + corpus-qc `utils.py` (commit f091abd). (3) **Encoding DB-fix** — dot-leader `ï€®`→`": "` + `Ã?` family (× ä ó ç í á ï ø) across 34 question rows (gitignored DB; residual 0). (4) **Fix #1 — `parse_blueprint` multi-page** (`ite_parser.py`): legacy 2018–2023 reports are 2-page but the parser hard-read `doc[0]` only → dropped ~37% of items (2022 123→195, 2023 125→198); now loops all pages + dedupes. (5) **Option C — Stage 1.8 blueprint DB cross-check + derive** (`ite_analyze_v2.py`): blueprint now derived from normalized DB by QID (authoritative), PDF-position kept as cross-check (compared via existing `BLUEPRINT_DB_TO_PDF` map so only true disagreements flag). (6) UTF-8 console crash fix (commit ae47b77). **MAJOR FINDING:** old-format 2022/2023 blueprint was ~70% mis-categorized by PDF-column position (geometry differs across ABFM eras: 2022 136/195, 2023 138/198 disagreements); new-format years agree 0–1/195 — validating both DB normalization AND the QID→item mapping. (7) QID-2024-0017 blueprint corrected `Acute Care and Diagnosis`→`Chronic Care Management` (the one new-format disagreement; per ABFM tie-breaker) → all 7 reports now 0 blueprint disagreements. All 7 resident analyses re-run + QC-verified (correct denominators, DB-authoritative blueprint, 0 encoding artifacts in JSON+DOCX, official YoY deltas). Closed DEFERRED-RESIDENT-ANALYSES-RERUN; opened 5 (BLUEPRINT-XCHECK-AUTOFIX, PATIENT-BASED-SYSTEMS-RESIDUAL, DOUBLED-QUOTE, C1-CONTROLS, REPORT-BUILDER-ENCODING). No new scripts (all edits to existing files). No PDF changes.

**Cleaned (2026-05-19, BATON 076):** Tier 2 walk-down + corpus-wide question-text + subscript-orphan cleanup. 8 distinct DB-write workflows applied (all backed up, all BATON 075 invariants preserved): A3 choices_empty re-extraction (42 QIDs — 27 OK + 15 PARTIAL_4OF5 source-PDF defect); A2 truncation no-op (23 heuristic false-positives); QID-2024-0067 enrichment backfill (keywords + concept_tags); QID-2024-0067 blueprint+body_system verified; 8th UMBRELLA review (+1 net is genuine signal); question_text embedded-answer-choice-block cleanup (42 questions); 2 specific choice text fixes (QID-2020-0162 "proficiency" append, QID-2021-0017 wandering "1c" merge); corpus-wide wandering-subscript cleanup (206 questions, 239 orphans removed, 176 medical-knowledge recoveries via 14 rules covering A1c/B12/FEV1/T4/H2-blocker/α1-/H2O/S3 gallop/HCO3/PaO2/PaCO2/Lp-PLA2). Final fidelity metrics all 0 (was 47/117/42/42/41). 5 new M2 scripts (reextract_a3_choices, reextract_a2_explanations, clean_question_text_contamination, render_partial_4of5_docx, fix_subscript_orphans). 4 deferred flags CLOSED, 2 NEW.

**Paused + Expanded (2026-05-18, BATON 072):** Two-part session.

*Part 1 — Device handoff:* Mikey opened a fresh Claude Code session, ran `/board-startup`, and asked for a corpus-integrity-qc progress recap before kicking off the BATON 071 testing carry-forward. After delivering the recap (built/remaining/known bugs, worktree path caveat for `run_qc.py`, fix-applier first-use plan) he pivoted the work to the Windows big rig. Fresh worktree `.claude/worktrees/awesome-chandrasekhar-3ae317` opened at hash `2079a2f`; initial BATON 072 written + first housekeeping commit `02a770f` + hash-backfill `4b8b878` + heads-up section added `a14dcaa`.

*Part 2 — session-housekeeping V3 / V3.1 upgrade:* While reviewing the PR #17 review block, Mikey rewrote the end-of-session GitHub-sync workflow. **V3 (`bb2e297`)** revised Item 12 end-to-end: agent now opens the PR, provides a structured review block in chat, **waits for explicit chat-level authorization** (`merge it` / `approved` / `go` etc.), then runs `gh pr merge` itself (no more "user merges in web UI"). 9-substep sequence: push → PR → review → wait → merge → prune → verify GitHub state → verify single-main → done. **V3.1 (`b125176`)** locked merge style to `--merge --delete-branch` — squash and rebase banned because every BATON pins intra-session commit hashes as part of its audit trail; squash destroys those hashes, rebase loses the session-boundary marker, merge-commit preserves both. **First exercise on PR #17** landed cleanly at `34867ce`: remote branch deleted, local branch deleted, worktree removed, single-main verified, all 5 session commit hashes still resolvable on main (proves the rationale). One wrinkle surfaced and deferred: `gh pr merge --delete-branch` from inside a worktree errors on local-side cleanup (DEFERRED-V3.2-WORKTREE-CHECKOUT-ORDER — 5-min SKILL.md `cd` fix).

*Net effect:* zero DB / PDF / pipeline-script changes; one substantive doc/skill change (`.claude/skills/session-housekeeping/SKILL.md` V2 → V3.1). All 17 BATON 071 deferred flags carry forward plus DEFERRED-V3.2-WORKTREE-CHECKOUT-ORDER newly opened. Top priority for the Windows resume remains the corpus-qc V1 testing pass.

**Promoted (2026-05-18, BATON 071):** 5 board_prep_intel-specific custom skills copied from the Claude Code plugin store into `.claude/skills/` so they resolve as bare slash commands (no `anthropic-skills:` prefix required): `board-startup/`, `body-system-qc/`, `article-citation-qc/` (with `references/` + `scripts/`), `baton-pipeline-qc/`, `repo-error-review/`. Project skill inventory now 9 entries (7 SKILL.md directories + 2 cowork `.skill` zips). Plugin-namespace versions remain available too — no override or shadow. Trigger: user reported `/board-startup` wasn't autocompleting. No DB/PDF/schema/script changes; pure `.claude/skills/` directory edit.

**Built (2026-05-15, BATON 070):** corpus-integrity-qc skill V1 completed — 10 new files in `.claude/skills/corpus-integrity-qc/` (5 Python scripts: `layer_a_text.py`, `layer_b_citation.py`, `generate_fixes.py`, `build_report.py`, `run_qc.py`; 5 markdown templates: `agents/README.md` + `agents/{text-fidelity,citation-linkage,structural-integrity}-auditor.md` + `agents/fix-applier.md`) + 1 modified (`SKILL.md` — V1 file layout + standalone Quickstart + agent template path lock). End-to-end smoke test against canonical DB produces 2,538 findings (Layer A: 158 / Layer B: 582 / Layer C: 1,798) split into 1,914 Tier 1 (auto-safe) + 66 Tier 2 (review, commented) + 558 Tier 3 (manual, no SQL). Layer B confirmed BATON 058 bug-fix: zero CRITIQUE_REF_MISSING_FROM_DB findings, the ~932 false-positive bug is gone. DEFERRED-CORPUS-QC-LAYERS-AB-D CLOSED; new flag DEFERRED-LAYER-A4-PDF-DIFF (carve-out). No DB writes, no PDF acquisition, no schema changes; M1/M2/M3 script counts unchanged.

**Cleaned (2026-05-15, BATON 069):** PROJECT_OVERHAUL fossil cleanup — removed all live `00_#PROJECT_OVERHAUL` path references (5 live config/code locations + 8 docs; the `#` character was a URL fragment delimiter and real bug risk for any URL-encoded path handling). Renamed `project_overhaul_state.md` → `project_session_log.md` in both `.auto-memory/` and `auto-memory-copies/` (via `git mv` — history preserved); old name was a fossil from the early M1-M5 module reorganization (~March 2026). CLAUDE.md H1 corrected from `# Memory — #PROJECT_OVERHAUL` to `# Memory — ITE Intelligence System`. Three intentional historical mentions preserved (not live refs): rebuild_structuring_guidelines.md description, project_new_architecture.md description, and renamed project_session_log.md H1 rename-note block. Commit `a3ef508` — 27 files, 2 git renames. No DB changes; no schema changes; no script additions; all row counts unchanged from BATON 068.

**Migrated + Built (2026-05-15, BATON 068):** Cowork → Claude Code migration validated; adopted Claude Code as primary workflow. Built corpus-integrity-qc skill scaffold (`.claude/skills/corpus-integrity-qc/` — 5 files: SKILL.md, references/qc_rules.md, references/fix_tiers.md, scripts/utils.py, scripts/layer_c_structural.py); 4-layer architecture (text fidelity / citation linkage / structural integrity / report+remediation) with parallel agent dispatch; Layer C functional and validated against canonical DB; Layers A, B, D + coordinator + subagent prompts deferred. Replaces buggy article-citation-qc (confirmed dict-overwrite bug in run_citation_qc.py lines 207–210). DB swap: Mac DB was 3 weeks stale (Apr 16, 1,998 articles / 2,485 xref); user staged canonical from gdrive at `00a_db_gdrive_landing/db/` (directory now serves as established landing zone for future gdrive DB pulls); swapped — old → `00_database/db/_archive_/ite_intelligence_stale_20260416.db`; new → canonical location; counts now match BATON 067 (2,206 articles / 2,710 xref). Layer C smoke test: 1,798 findings (1,797 derived-cache drift Tier 1 + 1 NEW ORPHAN_XREF bug at QID-2024-0067/ART-2073 — QID does not exist in questions table). No schema changes. New deferred flags: DEFERRED-CORPUS-QC-LAYERS-AB-D, DEFERRED-LAYER-C-CACHE-REBUILD, DEFERRED-ORPHAN-XREF-QID-2024-0067, DEFERRED-MAC-PDF-SYNC (Mac lags Windows 569 PDFs), DEFERRED-LOCKED-RULE-8-UPDATE (Rule 8 Windows-specific, needs Mac/Claude Code broadening).
