...
**Last Updated:** 2026-05-18 (BATON 074 — Skill shadow cleanup + archive reorganization. Completed lingering BATON 073 work: orphan worktree dir `.claude/worktrees/inspiring-cannon-e99bfb/` deleted at session start (Claude Code restart released cwd lock); 9 user-level skill shadows audited per-item with "best version" selected each: synced/promoted/extracted to project-level (`baton-pipeline-qc/SKILL.md` user → project for M3 extensions; `body-system-qc/references/` synced; `exa-research-search/` promoted; `methodology-scout/` extracted from `.skill` zip via temp-rename workaround); `article-citation-qc` retired (per BATON 068 corpus-integrity-qc replacement) — ALL variants archived to new `_archive_/deprecated_skills/article-citation-qc/` with provenance README + Cowork zip + BATON 073 stray scripts consolidated; orphan investigation file rescued to `_archive_/design_docs/` (folder renamed from `methodology_notes/`); full user-level snapshot to `_archive_/deprecated_skills/user_level_shadow_copies_2026-05-18/` with per-skill decisions README; classifier blocked `Remove-Item` on user-level dirs twice → workaround via `Move-Item` to Desktop consolidation folder + user manual Recycle Bin drop; `~/.claude/skills/` now EMPTY (9 entries fully retired with full archive preservation). Bonus archive scan: 4 obviously-stale files → new `_archive_/delete_me_051826/` staging (check_no_emails.py, stale README_PROJECT.md, stale README_canonical.json, ancient session_housekeeping_20260403.json). DEFERRED-ORPHAN-WORKTREE-DIR-CLEANUP CLOSED. DEFERRED-USER-LEVEL-SKILLS-AUDIT CLOSED. No DB / PDF / pipeline-script changes — row counts unchanged from BATON 073.)
**Status:** Current — Mac local PDFs lag Windows canonical by 567 files (Mac active total 973 vs Windows canonical 1,540: VC_fail Mac 630 / Win 1,056; VC_pass Mac 168 / Win 309; local_lite 117 + right_click 58 + AAFP 15 + ite_exams 16 all synced); M1 build = 8 + maintain = 38 (no script-count change BATON 074); M2 = 75py + 6js + 1json (scripts/) + 4py (core/) + 7py (engines/) + 6py (utils/) + 1py (M2 root); M3 = 55py + 4js + 7 json config; M4 = 1py; M5 scaffold = 3py + 35ts/tsx + 5sql; DB stable (2,206 articles, 1,639 ITE questions, 1,221 AAFP questions, qid_art_xref 2,710, article_icd10 4,959, question_icd10 5,774, clinical_pathways 4,959, intersection_centroid_vec 158); `.claude/skills/` exposes **10 project-level entries** post-cleanup (8 SKILL.md dirs: board-startup, body-system-qc +references/, baton-pipeline-qc (synced), repo-error-review, corpus-integrity-qc V1, session-housekeeping V3.2, **methodology-scout NEW BATON 074**, **exa-research-search NEW BATON 074**) + 2 Cowork zips (custom-question-set.skill + ite-exam-series.skill); article-citation-qc REMOVED from project (deprecated). `~/.claude/skills/` empty (full archive preservation in `_archive_/deprecated_skills/`).
