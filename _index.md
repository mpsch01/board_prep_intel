...
**Last Updated:** 2026-05-19 (BATON 075 — Corpus-integrity-qc V1 testing pass + DB-write debut. Ran `run_qc.py` end-to-end (first standalone run), fixed 3 substantive bugs in-flight in the skill itself: (1) PROJECT_ROOT off-by-one in all 5 entry-point scripts (`.parent×5` → `×4`; SKILL.md doc also corrected), (2) Windows cp1252 console crash on `✓` chars (new `setup_utf8_stdout()` helper in utils.py + UTF-8 on 8 open() + 3 subprocess.run() calls), (3) A1 ENCODING_ARTIFACT no-op on `choices` JSON column — discovered SQLite *does* interpret `\u` escapes in single-quoted string literals (despite docs), fixed via new `_sql_json_escape_expr()` building `char(92) || 'uXXXX'` literals. Applied 1,914 Tier 1 statements via inline fix-applier workflow (atomic BEGIN/COMMIT, 172 MB pre-apply backup, 6 verify COUNTs); findings dropped 2,538 → 624, Layer C went 1,798 → **0**. Investigated lone ORPHAN_XREF — question QID-2024-0067 (acute HIV diagnostic) existed in source PDFs but had been dropped during ingestion; recovered + inserted with primary fields populated (questions count 1,639 → 1,640). DB invariants now hold: `sum(citation_count) == COUNT(xref) == 2,710` and `count(articles with citations) == count(distinct article_id in xref) == 1,982`. 3 deferred flags CLOSED (DEFERRED-LAYER-C-CACHE-REBUILD, DEFERRED-ORPHAN-XREF-QID-2024-0067, DEFERRED-V1-ENCODING-CHOICES-JSON-BUG opened+closed same session); 4 NEW (DEFERRED-LAYER-A5-LANGUAGE-INTEGRITY, DEFERRED-LAYER-A6-RENDER-FIDELITY, DEFERRED-QID-2024-0067-ENRICHMENT, DEFERRED-LAYER-B-UMBRELLA-PROMOTION-REVIEW). 8 corpus-integrity-qc files MODIFIED (7 scripts/*.py + SKILL.md). No M1/M2/M3/M4/M5 script-count changes. No PDF changes.)
**Status:** Current — Mac local PDFs lag Windows canonical by 567 files (Mac active total 973 vs Windows canonical 1,540: VC_fail Mac 630 / Win 1,056; VC_pass Mac 168 / Win 309; local_lite 117 + right_click 58 + AAFP 15 + ite_exams 16 all synced); M1 build = 8 + maintain = 38 (no script-count change BATON 075); M2 = 75py + 6js + 1json (scripts/) + 4py (core/) + 7py (engines/) + 6py (utils/) + 1py (M2 root); M3 = 55py + 4js + 7 json config; M4 = 1py; M5 scaffold = 3py + 35ts/tsx + 5sql; DB UPDATED (2,206 articles unchanged; 1,**640** ITE questions +1 QID-2024-0067 recovered; 1,221 AAFP questions unchanged; qid_art_xref 2,710 unchanged; article_icd10 4,959 unchanged; question_icd10 5,774 unchanged; clinical_pathways 4,959 unchanged; intersection_centroid_vec 158 unchanged — cache columns on `articles` table now correct per Tier 1 apply); `.claude/skills/` exposes **10 project-level entries** (8 SKILL.md dirs: board-startup, body-system-qc, baton-pipeline-qc, repo-error-review, **corpus-integrity-qc V1.0.1 (Windows-hardened BATON 075)**, session-housekeeping V3.2, methodology-scout, exa-research-search) + 2 Cowork zips (custom-question-set.skill + ite-exam-series.skill); `~/.claude/skills/` empty.
