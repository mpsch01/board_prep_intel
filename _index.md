...
**Last Updated:** 2026-05-15 (BATON 070 — corpus-integrity-qc skill V1 complete: Layers A, B, D + coordinator + 4 agent templates built and end-to-end smoke-tested; no DB/PDF/schema changes (DB row counts unchanged))
**Status:** Current — Mac local PDFs lag Windows canonical by 569 files (Mac active total 971 vs Windows canonical 1,540: VC_fail Mac 628 / Win 1,056; VC_pass Mac 168 / Win 309; local_lite 117 + right_click 58 + AAFP 15 + ite_exams 16 all synced); M1 build = 8 + maintain = 38 (no script-count change BATON 070); M2 = 75py + 6js; M3 = 55py + 4js + 1 json config + abfm_reference_2024.json; M5 scaffold = 3py + 31ts/tsx + 5sql; DB stable (2,206 articles, 1,639 ITE questions, 1,221 AAFP questions, qid_art_xref 2,710, article_icd10 4,959, question_icd10 5,774, clinical_pathways 4,959, intersection_centroid_vec 158); .claude/skills/corpus-integrity-qc/ now COMPLETE V1 (15 files: SKILL.md + 2 references + 7 scripts + 5 agent templates; A/B/C/D + coordinator all functional; A4 PDF-diff deferred to V1.1); skills_abilities/ includes ite-score-analyzer-v2/ plugin + session-housekeeping agents/.
...
```
board_prep_intel/
├── BATON_active_070_20260515_corpus_qc_skill_v1.md  — active BATON
