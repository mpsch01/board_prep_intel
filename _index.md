...
**Last Updated:** 2026-05-18 (BATON 072 — device-handoff pause + mid-session expansion; orientation + corpus-qc status recap, then work pivoted to the Windows big rig. **Mid-session:** session-housekeeping skill upgraded to V3.1 (chat-level auth gate + agent runs `gh pr merge`; merge style locked to `--merge --delete-branch` to preserve BATON intra-session hash references). PR #17 merged via the new V3.1 flow at `34867ce`; all 5 session commits preserved on main. No DB/PDF/pipeline-script changes (row counts unchanged from BATON 071); new deferred flag DEFERRED-V3.2-WORKTREE-CHECKOUT-ORDER.)
**Status:** Current — Mac local PDFs lag Windows canonical by 567 files (Mac active total 973 vs Windows canonical 1,540: VC_fail Mac 630 / Win 1,056; VC_pass Mac 168 / Win 309; local_lite 117 + right_click 58 + AAFP 15 + ite_exams 16 all synced); M1 build = 8 + maintain = 38 (no script-count change BATON 071); M2 = 75py + 6js; M3 = 55py + 4js + 1 json config + abfm_reference_2024.json; M5 scaffold = 3py + 31ts/tsx + 5sql; DB stable (2,206 articles, 1,639 ITE questions, 1,221 AAFP questions, qid_art_xref 2,710, article_icd10 4,959, question_icd10 5,774, clinical_pathways 4,959, intersection_centroid_vec 158); .claude/skills/ now exposes 9 project-level entries (board-startup, body-system-qc, article-citation-qc, baton-pipeline-qc, repo-error-review NEW BATON 071; plus corpus-integrity-qc V1 + session-housekeeping V2 directories + custom-question-set.skill + ite-exam-series.skill zips); skills_abilities/ includes ite-score-analyzer-v2/ plugin + session-housekeeping agents/.
