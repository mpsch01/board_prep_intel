...
**Last Updated:** 2026-05-18 (BATON 073 — V3.2 workflow transition: no worktrees ever. Windows resume after BATON 072 Mac→Windows handoff. Pre-flight passed (DB 2206/1639/2710 verbatim). Discovered + cleaned: 4 stale Claude worktrees (3 removed + branches deleted), CLAUDE.md unresolved stash-pop conflict markers in main checkout (resolved), 2 stray untracked deprecated M3 scripts (archived to `_archive_/legacy_article_citation_qc/`), 3-way session-housekeeping skill divergence (user-level V2 shadowing project-level V3.1; user-level deletion deferred to user manual action per Locked Rule 8). CLAUDE.md Session-Housekeeping Skill section + SKILL.md upgraded V3.1 → V3.2 (no worktrees policy + rationale + legacy-cleanup steps). DEFERRED-V3.2-WORKTREE-CHECKOUT-ORDER CLOSED (obviated). DEFERRED-USER-LEVEL-SKILL-DELETION NEW. No DB / PDF / pipeline-script changes — row counts unchanged from BATON 072.)
**Status:** Current — Mac local PDFs lag Windows canonical by 567 files (Mac active total 973 vs Windows canonical 1,540: VC_fail Mac 630 / Win 1,056; VC_pass Mac 168 / Win 309; local_lite 117 + right_click 58 + AAFP 15 + ite_exams 16 all synced); M1 build = 8 + maintain = 38 (no script-count change BATON 073); M2 = 75py + 6js + 1json (scripts/) + 4py (core/) + 7py (engines/) + 6py (utils/) + 1py (M2 root); M3 = 55py + 4js + 7 json config; M4 = 1py; M5 scaffold = 3py + 35ts/tsx + 5sql; DB stable (2,206 articles, 1,639 ITE questions, 1,221 AAFP questions, qid_art_xref 2,710, article_icd10 4,959, question_icd10 5,774, clinical_pathways 4,959, intersection_centroid_vec 158); `.claude/skills/` exposes 9 project-level entries (board-startup, body-system-qc, article-citation-qc, baton-pipeline-qc, repo-error-review; corpus-integrity-qc V1; **session-housekeeping V3.2 (BATON 073)** — no worktrees ever; custom-question-set.skill + ite-exam-series.skill zips); `~/.claude/skills/session-housekeeping/` Apr-16 V2 stale shadow pending user deletion (DEFERRED-USER-LEVEL-SKILL-DELETION).
