...
**Last Updated:** 2026-05-19 (BATON 076 — Tier 2 walk-down + corpus-wide question-text and subscript-orphan cleanup. 8 distinct DB-write workflows applied this session (all backed up; all BATON 075 invariants preserved): (1) A3 choices_empty re-extraction for 42 QIDs (27 OK 5-of-5 + 15 PARTIAL_4OF5 — ABFM source-PDF defect missing choice E; correct_letter in {A,B,C,D} for all 15 per user's data-completeness bar); (2) A2 truncation-candidate re-extraction → all 23 ALREADY_FULL (heuristic false-positives; opens DEFERRED-LAYER-A2-HEURISTIC-TUNING); (3) QID-2024-0067 enrichment backfill (TF-IDF keywords + concept_tags via Sonnet 4.6 / 1 API call / ~$0.01); (4) blueprint+body_system verification (Acute Care and Diagnosis + Hematologic/Immune hold per precedent); (5) 8th UMBRELLA review — +1 net is honest signal (ART-0427 USPSTF Testicular Cancer + ART-0789 Harrison's textbook both legitimate); (6) question_text contamination cleanup for 42 QIDs (embedded answer-choice block + trailing empty A) B) C) D) E) slots + Item #NN residue) + 2 specific choice fixes; (7) wandering-subscript orphan discovery → built fix_subscript_orphans.py with 14 medical-knowledge recovery rules (A1c/B12/FEV1/T4/H2-blocker/α1-/H2O/S3 gallop/HCO3/PaO2/PaCO2/Lp-PLA2); (8) corpus-wide subscript cleanup for 206 questions (239 orphans removed, 176 recoveries). **Final corpus state — all fidelity metrics at 0**: 0 question_text subscript orphans (was 47), 0 explanation subscript orphans (was 117), 0 empty choices (was 42), 0 empty correct_text (was 42), 0 embedded answer-choice blocks (was 41). 4 deferred flags CLOSED (QID-2024-0067-ENRICHMENT, LAYER-B-UMBRELLA-PROMOTION, A3-SOURCE-PDF-MISSING-E, SUBSCRIPT-ORPHAN-CORPUS); 2 NEW (LAYER-A2-HEURISTIC-TUNING, CORPUS-QC-LAYER-A7-A8 = embedded-choices + subscript-orphan auto-detection checks for V1.1).)
**Status:** Current — Mac local PDFs lag Windows canonical by 567 files (Mac active total 973 vs Windows canonical 1,540; tier counts unchanged BATON 076); M1 build = 8 + maintain = 38 (no script-count change BATON 076); M2 = **80py** (+5 BATON 076: reextract_a3_choices.py + reextract_a2_explanations.py + clean_question_text_contamination.py + render_partial_4of5_docx.py + fix_subscript_orphans.py) + 6js + 1json (scripts/) + 4py (core/) + 7py (engines/) + 6py (utils/) + 1py (M2 root); M3 = 55py + 4js + 7 json config; M4 = 1py; M5 scaffold = 3py + 35ts/tsx + 5sql; DB UPDATED (~250 questions touched across 8 atomic write workflows; row counts on all 13 tracked tables unchanged from BATON 075 — only column values updated); `.claude/skills/` exposes **10 project-level entries** (unchanged BATON 076); `~/.claude/skills/` empty.
