...
def generate_sql(findings: list[dict]) -> tuple[str, dict]:
    ...
    if qid_findings_exact:
        # Group by (qid, exam_year) to detect multi-reference QIDs
        qid_groups = defaultdict(list)
        for f in qid_findings_exact:
            key = (f.get('qid', ''), f.get('exam_year', ''))
            qid_groups[key].append(f)

        multi_ref_count = sum(1 for g in qid_groups.values() if len(g) > 1)

        lines.append(f"-- ══════════════════════════════════════════════════════")
        lines.append(f"-- QID_MISMATCH fixes — EXACT MATCH ONLY ({len(qid_findings_exact)} citations, {len(qid_groups)} QIDs)")
        lines.append(f"-- Critique citation matched DB article with match_score = 1.0.")
        lines.append(f"-- Single-ref QIDs: UPDATE existing row.")
        lines.append(f"-- Multi-ref QIDs ({multi_ref_count}): UPDATE first row, INSERT additional rows.")
        lines.append(f"-- {len(qid_findings_fuzzy)} fuzzy-matched mismatches are in the report only (manual review).")
        lines.append(f"-- ══════════════════════════════════════════════════════")
        lines.append(f"")

        for (qid, year), group in sorted(qid_groups.items()):
            current = group[0].get('current_value', '')
            first = group[0]

            if len(group) == 1:
                corrected = first.get('corrected_value', '')
                lines.append(f"-- {qid} (exam {year})")
                lines.append(f"-- DB had: {current} | Critique says: {corrected}")
                lines.append(f"-- Citation: '{(first.get('clean_ref') or '')[:80]}'")
                lines.append(
                    f"UPDATE qid_art_xref SET article_id = '{escape_sql(corrected)}' "
                    f"WHERE qid = '{qid}' AND exam_year = {year} AND article_id = '{escape_sql(current)}';"
                )
            else:
                # Multi-reference: UPDATE first, INSERT the rest
                lines.append(f"-- {qid} (exam {year}) — {len(group)} references (multi-ref QID)")
                lines.append(f"-- DB had: {current} — replacing with {len(group)} critique citations")
                corrected_first = first.get('corrected_value', '')
                lines.append(f"-- [1] UPDATE: {current} → {corrected_first}")
                lines.append(f"-- Citation: '{(first.get('clean_ref') or '')[:80]}'")
                lines.append(
                    f"UPDATE qid_art_xref SET article_id = '{escape_sql(corrected_first)}' "
                    f"WHERE qid = '{qid}' AND exam_year = {year} AND article_id = '{escape_sql(current)}';"
                )
                for i, f in enumerate(group[1:], start=2):
                    corrected_extra = f.get('corrected_value', '')
                    lines.append(f"-- [{i}] INSERT: {corrected_extra}")
                    lines.append(f"-- Citation: '{(f.get('clean_ref') or '')[:80]}'")
                    lines.append(
                        f"INSERT OR IGNORE INTO qid_art_xref (qid, article_id, exam_year) "
                        f"VALUES ('{escape_sql(qid)}', '{escape_sql(corrected_extra)}', {year});"
                    )

            lines.append(f"")
            stats["QID_MISMATCH_exact"] += len(group)
