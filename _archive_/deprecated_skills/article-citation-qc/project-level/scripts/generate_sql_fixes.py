...
Usage:
    python generate_sql_fixes.py \
        --qc-results /path/to/qc_results.json \
...
def generate_sql(findings: list[dict]) -> tuple[str, dict]:
    """Generate SQL statements from QC findings. Returns (sql_text, stats)."""
    ...
    # ── QID_MISMATCH (exact matches only) ────────────────────────────────────
    qid_findings_exact = [
        f for f in findings
...
def main():
    ...
    parser.add_argument("--qc-results", required=True, help="Path to qc_results.json")
    parser.add_argument("--output-dir", required=True, help="Directory for article_qc_fixes.sql")
