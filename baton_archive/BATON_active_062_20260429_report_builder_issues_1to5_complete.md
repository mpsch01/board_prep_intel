...
### Word Doc Style Standard (Locked Rule 14)
**Decision:** All Python `.docx` generation scripts must use `word_doc_defaults.py` (St. Luke's palette, Aptos font, gold accent) unless explicitly overridden.
**Rationale:** Two guide scripts existed in parallel (JS and Python versions). Python + word_doc_defaults.py is now the canonical path — consistent with Rule 5 (no de novo JS) and produces consistent visual identity across all project Word outputs.
**Location:** `03_module.3_analyst/scripts/word_doc_defaults.py`
**Added to CLAUDE.md:** Rule 14, 2026-04-29.
