...
ITE_BASE = SCRIPT_DIR.parent  # 03_ite_exam/

ENRICHED_JSON = ITE_BASE / "02_working" / "ite_2018_2019_enriched.json"
MASTER_JSON = ITE_BASE / "03_database" / "ite_questions_clean.json"
MASTER_JSON_BACKUP = ITE_BASE / "03_database" / f"ite_questions_clean_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

INTEL_BASE = ITE_BASE.parent / "02_ite_intelligence"
DB_PATH = INTEL_BASE / "db" / "ite_intelligence.db"
DB_BACKUP = INTEL_BASE / "db" / f"ite_intelligence_pre2018_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.db"

LOG_DIR = INTEL_BASE / "logs"
