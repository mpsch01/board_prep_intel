...
"""
convert_pdfs_to_json.py  v1.2 (post-codon migration)

Convert a directory of PDFs into:
- *_extracted.json   (metadata + navigator_v1 scaffold + ingest pointers, flat in out_dir)
- raw_txt/*.raw.txt  (full extracted text, subfolder)
- manifest.json      (index of outputs)

v1.2 changes:
  - Output JSON suffix: _extracted.json (matches enricher v4 glob pattern)
  - Default --out_dir: relative to --pdf_dir parent (no hardcoded Windows paths)
  - source.file_name preserves original PDF filename including codon (#@#ART-XXXX@#@)

USAGE (PowerShell):
  python scripts/convert_pdfs_to_json.py --pdf_dir "clinical_guidelines/01_pdf_guideline_library/pdf_codon"

Optional:
  python scripts/convert_pdfs_to_json.py --pdf_dir "path/to/pdfs" --out_dir "path/to/outputs"

NOTE: Default --out_dir resolves to the sibling '03_enriched_JSON' folder under
      clinical_guidelines/, regardless of subfolder depth under 01_pdf_guideline_library/.
"""
...
ENGINE_VERSION = "ite_pdf_to_json_v1.2"
...
def build_json_payload(
    *,
    ...
    """
    Create JSON with:
    - core 'source' fields (file_name preserves codon for enricher Strategy 0)
    - 'ingest' pointers
    - 'navigator_v1' scaffold per Guideline Navigator v1.0 concept
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Output folder root (JSONs written directly here, raw_txt/ subfolder for text). "
             "Defaults to sibling '03_enriched_JSON' folder next to --pdf_dir.",
    )
    args = parser.parse_args()

    pdf_dir = os.path.abspath(args.pdf_dir)

    # Default out_dir: sibling of '01_pdf_guideline_library' (e.g. clinical_guidelines/03_enriched_JSON)
    # Walks up from pdf_dir until it finds '01_pdf_guideline_library', then goes to sibling.
    if args.out_dir:
        out_dir = os.path.abspath(args.out_dir)
    else:
        # Navigate up to the 01_pdf_guideline_library level, then to its parent
        d = pdf_dir
        while os.path.basename(d) != "01_pdf_guideline_library" and d != os.path.dirname(d):
            d = os.path.dirname(d)
        out_dir = os.path.join(os.path.dirname(d), "03_enriched_JSON")
    ...
    print("=" * 60)
    print(f"PDF-to-JSON Extractor  v1.2")
    ...
    for fname in pdfs:
        ...
        try:
            ...
            json_path = os.path.join(out_dir, f"{stem}_extracted.json")
            ...
            print(f"  [OK] {fname[:55]} -> {os.path.basename(json_path)}")
